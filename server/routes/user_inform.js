...
router.post('/onRegister', (req, res) =>
{
    ...
    db.query(sql1, user_id, (err, data) => {
        if(!err) {
            // 결과값이 동일하다면
            if(data[0].result == 1) {
                res.send({ 'msg': '이미 가입된 아이디입니다.'})
            } else {
                const sql2 = 'INSERT INTO user_inform (user_id, user_pw, user_name) VALUES(?,?,?)'
                db.query(sql2, [user_id, user_pw, user_name], (err, result) => {
                    if(err) {
                        console.log("회원가입 실패");
                    }else {
                        console.log("회원가입 성공");
    ...
    router.post('/write', (req, res) => {
        ...
        const sql1 = "INSERT INTO freeboard (subject, content) VALUES (?,?)";
        db.query(sql1, [subject, content], (err, result) => {
            res.send('success');
        });
    })
